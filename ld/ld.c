...
coffhead_t *read_coffhead(char *imgx)
{
	...
	memcpy(p->sig,img,4);
	p->entno	= e_read32(img+ 4);
	p->repaddr	= e_read32(img+ 8);
	p->repno	= e_read32(img+12);
	...
	for(int i = 0; i < p->entno; i++) {
		...
		for(int j = 0; j < 16; j++) {
			p->symbols[i].attribute[j] = e_read32(img+16+i*SIZE_SYMBOL+64+4*j);
		}
	}
